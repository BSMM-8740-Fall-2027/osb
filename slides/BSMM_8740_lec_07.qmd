...
-   Last week we explored time series data, how they differ from typical cross-sectional or panel data, & how the differences matter for data analytics methods.

-   We looked at breaking time series into components, and modeling autoregressive processes.
...
-   We Introduce the fundamental problems of causal inference and the biases of some intuitive estimators.
...
One calculation we can do is to contrast the outcomes of the treated with the outcomes of the untreated.
:::
...
\end{align*}
$$ and $Y_{i}^{0}D_{i}$ is unobserved when $D_i=1$.
...
In this example, the $\Delta_\text{WW}$ estimator is a biased estimate of the $\text{ATE}$ estimand because the untreated are bigger spenders than the treated on average.
:::
...
we say the treated and untreated populations are [**exchangeable**]{.underline}.
...
Then we can identify the $\text{ATE}$, where $\text{ATE}=\pi\text{ATT}+(1-\pi)\text{ATU}$, and $\pi$ is the proportion treated in the population.
...
-   The FPSI states that, even if we have an estimator $E$ that identifies $\Delta_{-}^{Y}$ in the population, we cannot observe $E$ because we only have access to a finite sample of the population.
