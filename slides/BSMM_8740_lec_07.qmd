...
-   We showed how to break time series into ETS components, and how to model autoregressive processes.
...
-   We understand of how econometric methods recover treatment effects,
...
For binary treatments, treatment allocation is represented by the variable $D_i$, where $D_i=1$ if unit $i$ receives the treatment and $D_i=0$ if unit $i$ does not receive the treatment.
...
:::: {.column width="50%"}
::: {style="font-size: large"}
In practice, we only know the outcomes corresponding to the treatment. In this case an [**estimator**]{.underline} of the $\text{ATE}$ is the with-without estimator ($\Delta_{WW}$) which computes the difference between the average outcomes of the treated and untreated units.
:::
::::
...
In this example, the $\Delta_\text{WW}$ estimator is a biased estimate of the $\text{ATE}$ estimand; the bias is due to the untreated being bigger spenders than the treated, on average.
:::
...
We can eliminate the bias when particular assumptions are true.
...
-   [**common support**]{.underline}, meaning both treated and untreated units exist in the population (implicit)
:::
...
If the identification assumption holds, and outcomes are (effectively) independent of the treatment assignment, then
...
::: {style="font-size: x-large"}
In our first example we had a balanced data set: the same number of units are treated and untreated. If this is not the case, then
...
where: (i) $n_x = |\{i : X_i = x\}|$ , (ii) $n_{xw} = |\{i : X_i = x, D_i = w\}|$
:::
...
Paths that permit correlation between two variables are called ***open paths***. Paths that do not tpermt correlation are called ***closed paths***. Forks and chains are open, while colliders are closed.
:::

## DAGs - forks

::: {style="font-size: 80%"}
So, should we adjust[^3] for confounder `q`, i.e. include `q` in our model? That depends on the nature of the path.
...
[^3]: recall that in regression, controlling for a variable is used to isolate the relationship between your main variables of interest while holding the control variable constant.
