...
subtitle: "BSMM8740-2-R-2027F [WEEK - 6]"
author: "L.L. Odette"
...
-   Time series models
    -   ARIMA
    -   ETS
-   Time series tools: `timetk`, `modeltime`
...
::::: columns
::: {.column width="50%" style="font-size: large"}
```{r}
#| echo: true
#| code-fold: true
...
The roots of the polynomial can be computed using `polyroot`, passing in a vector of polynomial coefficients in increasing order. The magnitudes of the eigenvalues canbe found using the base function `Mod`.
...
-   An ARIMA model is denoted as ARIMA(p, d, q), where: p is the number of lag observations in the model (AR part), d is the number of times that the raw observations are differenced (I part), and q is the size of the moving average window (MA part).
:::
