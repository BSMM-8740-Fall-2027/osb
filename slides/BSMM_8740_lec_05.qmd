...
where $w$ is the weight vector (normal to the hyperplane) and $b$ is the bias term[^6].

[^6]: if $w^T x + b = 0$ separates classes, so does $2w^T x + 2b = 0$ so we fix the scale by normalizing all vectors by $\frac{1}{||w||}$.
