...
librarian::shelf(tidyverse, broom, rsample, ggdag, causaldata, magrittr, gt, gtExtras, halfmoon, ggokabeito, ggplot2, r-causal/propensity, r-causal/causalworkshop, survey, cardx, ddsjoberg/gtsummary, gapminder, fixest, haven)
...
1.  the estimator you have chosen (i.e. are the weights bounded?)
...
mt0 <- # untreated knn model for predicting recovery
  caret::knnreg(x = untreated |> dplyr::select(sex,age,severity), y = untreated$recovery, k=1)
mt1 <- # treated knn model for predicting recovery
...
predicted |>   
  dplyr::slice_head(n=5) |> 
  gt::gt() |> 
  gt::cols_label( recovery = "recovery (treated)", match = "recovery (untreated)" )|> 
  # gt::cols_width(recovery ~ px(100), match ~ px(100), everything() ~ px(90)) |> 
  gt::tab_footnote(footnote = "matched on treated covariates: sex, age and severity", locations = gt::cells_column_labels(columns = match)) |> 
...
The intuition will apply to the ATU as well.
:::
...
predicted |>   
  ...
  gt::gt() |> 
  gt::cols_label( recovery = "recovery (treated)", match = "recovery (untreated)" )|> 
  #gt::cols_width(recovery ~ px(100), match ~ px(100), bias_correct ~ px(100), everything() ~ px(85)) |> 
  gt::tab_footnote(footnote = "matched on treated covariates: sex, age and severity", locations = gt::cells_column_labels(columns = match)) |> 
  gt::fmt_number(columns = c('sex','age','severity'), decimals = 6) |>
  gtExtras::gt_theme_espn() # |> gt::as_raw_html()
```
...
::: {style="font-size: x-large"}
Doesn’t this defeat the point of matching? If we have to run a linear regression anyway, why don’t we just use the regression instead of this complicated model.

But this linear regression that we are fitting doesn’t extrapolate on the treatment dimension to get the treatment effect. Instead, its purpose is just to correct bias.
...
::: {style="font-size: large"}
Finally, we can say with confidence that our medicine does indeed lower the time someone spends at the hospital. The ATE estimate here is just a little bit lower than our algorithm, due to the difference in tie breaking of matches of the knn implementation and the `Matching` R package.
:::
...
-   Why does that work?
-   We want to control for "group/individual." So just put in a control for "group/individual."
...
-   Two main types:
    -   unit fixed effects: stable traits of each unit, e.g. depending store location, manager skill, customer base
    -   time fixed effects: shocks hitting every unit at the same time, e.g. seasonality, holidays, supply chain effects
    -   e.g. two periods: $sales_{it}=unitFE_i+timeFE_t+treatment_{it}+error_{it}$
:::
...
-   It seems that states are more or less likely to implement reform in a way that's correlated with the level of robbery they already had
...
-   Who knows what!
-   Our DAG has `reform -> robberies` and `reform <- state -> robberies`, which is something we can address with fixed effects.
...
[^2]: see [John Snow's Cholera Hypothesis](https://mixtape.scunning.com/09-difference_in_differences#john-snows-cholera-hypothesis)
...
-   so $\left(Y_{i2}(1)-Y_{i1}(1)\right) - \left(Y_{i2}(0)-Y_{i1}(0)\right)=\delta\equiv\hat{\tau}_{DiD}$

## DiD Causal Inference

::: {style="font-size: x-large"}
Under what conditions does this estimator have a causal interpretation?
...
\begin{align*}
\hat{\tau}_{DiD} & =E[Y_{i2}(1)-Y_{i1}(1)]-E[Y_{i2}(0)-Y_{i1}(0)]\\
 & =E[{\color{blue}Y_{i2}(1)}-{\color{green}Y_{i1}(1)}|D_{i}=1]-E[{\color{orange}Y_{i2}(0)}-{\color{magenta}Y_{i1}(0)}|D_{i}=0]+\\
 & E\left[{\color{cyan}Y_{i2}(1)}|D_{i}=0\right]-E\left[Y_{i2}(1)|D_{i}=0\right]\\
 & =E[{\color{blue}Y_{i2}(1)}|D_{i}=1]-E\left[Y_{i2}(1)|D_{i}=0\right]+\\
 & E[{\color{cyan}Y_{i2}(1)}-{\color{green}Y_{i1}(1)}|D_{i}=0]-E[{\color{orange}Y_{i2}(0)}-{\color{magenta}Y_{i1}(0)}|D_{i}=0]
\end{align*}
$$
:::

## DiD Causal Inference {visibility="hidden"}
...
1.  No anticipation: $$E[Y_{i1}(1)|D_{i}=1] = E[Y_{i1}(0)|D_{i}=0]$$

2.  Parallel trends: $$0=E[Y_{i2}(1)-Y_{i1}(1)|D_{i}=1]-E[Y_{i2}(0)-Y_{i1}(0)|D_{i}=0]$$
...
Our estimator has two sources of bias specific to the estimator, no anticipation and parallel trends:
...
## Multiyear 2x2 example

::: panel-tabset
## data

```{r}
#| label: load, filter, and create columns for 2x2 example
#| echo: true
# Load the data
...
```{r}
#| label: Calculate means for different groups, simple 2x2 example
#| echo: true
#| code-fold: true
#| message: false
...
(mean_values <- data %>%
  ...
  dplyr::ungroup() |> 
  gt::gt() |> 
  gt::fmt_number(columns = mean_l_homicide, decimals = 4)
...
```{r}
#| label: regression for 2x2 example
#| echo: true
#| code-fold: true
...
-   Repeated Cross-Sections: different units sampled at each time (e.g., Census, Current Population Survey)
    ...
    -   Violations of parallel trends can arise differently across these longitudinal data types.
...
## Repeated cross-sections {visibility="hidden"}
...
-   To be causal, 2x2 relies on parallel trends (PT) and no anticipation (NA)
...
As before, for our DiD estimator to be a causal estimate (ATT) we need (now conditional on covariates X)
...
$$\hat{\tau}_{DiD} = \left(\bar{Y}_2(1) - \bar{Y}_1(1)) - (\bar{Y}_2(0) - \bar{Y}_1(0)\right) $$

becomes

$$\hat{\tau}_{DiD} = \left(\bar{Y}_2(1) - \bar{Y}_1(1)\right) - \left(\hat{\mu}_{X,2}(0) - \hat{\mu}_{X,1}(0)\right) $$
...
$$
\bar{Y}_t(d)=\sum_{i\vert D_i=d,T_i=t}Y_{it}/n_{d,t}
$$

and $\hat{\mu}_{x,t}(d)$ estimates the true, unknown

$$
\mu_{x,t}(d)\equiv\mathbb{E}\left[Y_y\vert D=d,X=x\right]
$$
...
$$
\begin{align*}
\mathbb{E}_{X,D}[\rho(D)\left(Y_{t=2}-Y_{t=1}\right)|X] & =\\
\mathbb{E}_{X}[\rho(D)\left(Y_{t=2}-Y_{t=1}\right)|X,D=1]\pi+\mathbb{E}_{X}[\rho(D)\left(Y_{t=2}-Y_{t=1}\right)|X,D=0]\left(1-\pi\right) & =\\
\mathbb{E}_{X}[\frac{1}{\pi}\left(Y_{t=2}-Y_{t=1}\right)|X,D=1]\pi+\mathbb{E}_{X}[\frac{-1}{\left(1-\pi\right)}\left(Y_{t=2}-Y_{t=1}\right)|X,D=0]\left(1-\pi\right) & =\\
\mathbb{E}_{X}[Y_{t=2}-Y_{t=1}|X,D=1]-E[Y_{t=2}-Y_{t=1}|X,D=0] & =\\
\mathbb{E}_{X}[Y_{t=2}^{1}-Y_{t=2}^{0}|X,D=1]
\end{align*}
$$
:::
...
```{r}
#| label: load data and add IPWs
#| code-fold: true
#| echo: true
#
panel <- readr::read_csv("../slides/data/coupon.csv", show_col_types = FALSE)
...
w_tbl <- ps_by_id |>
  dplyr::mutate(w = if_else(treat == 1, pbar/ps, (1 - pbar)/(1 - ps))) |>
  dplyr::select(id, w)

# Attach weights to the panel
panel_w <- panel |> dplyr::left_join(w_tbl, by = "id")
...
```{r}
#| label: estimate ATT from the event study
#| code-fold: true
#| message: false
#| warning: false
#| echo: true
...
## 2X2 DiD & covariates (IPW) {visibility="hidden"}
...
E[Y_{t=2}^{1}-Y_{t=1}^{0}|D=1] & =\int E[Y_{t=2}^{1}-Y_{t=1}^{0}|X,D=1]d\mathbb{P}(x\vert D=1)\\
 ...
 & =E\left[\rho(D)\left(Y_{t=2}-Y_{t=1}\right)\frac{\mathbb{P}(D=1\vert X)}{\mathbb{P}(D=1)}\right]\\
 & =E\left[\frac{Y_{t=2}-Y_{t=1}}{\mathbb{P}(D=1)}\frac{D-\mathbb{P}(D=1\vert X)}{1-\mathbb{P}(D=1\vert X)}\right]
\end{align*}
$$
:::

## 2X2 DiD & covariates (DR) {visibility="hidden"}
...
tibble::tribble(
  ~'Feature',~'2×2 Regression (TWFE) DiD',~'Fixed-Effects DiD'
  #--|--|----
...
## Event Studies & Fixed Effects {visibility="hidden"}

In the case where there are no effects and e.g. $Y_{it}(1)=Y_{it}(0)=\gamma\cdot t + \epsilon_{it}$ and $\mathbb{E}[\epsilon_{it}]=0$
...
$$
\beta_{FWL}=\tau_{gt}=\frac{\text{cov}(\tilde{D}_{gt},\tilde{y}_{igt})}{\text{var}(\tilde{D}_{gt})}
$$

This is equal to the alternative formulation using the mean of the outcomes and the treatment indicator.

$$
\beta_{FWL}=\tau_{gt}=\frac{\text{cov}(\tilde{D}_{gt},\hat{y}_{igt})}{\text{cov}(\tilde{D}_{gt},D_{gt})}
$$
:::
...
where $X_1$ and $X_2$ are feature matrices (one row per feature and one column per observation) and $\hat{\beta_1}$ and $\hat{\beta_2}$ are row vectors. You can get the exact same $\hat{\beta_1}$ parameter by doing[^3]
