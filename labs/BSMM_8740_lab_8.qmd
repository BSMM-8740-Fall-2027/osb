...
-   To complete the lab, log on to **your** github account and then go to the class [GitHub organization](https://github.com/bsmm-8740-fall-2027) and find the **2027-lab-8-\[your github username\]** repository .

    Create an R project using your **2027-lab-8-\[your github username\]** repository (remember to create a PAT, etc.) and add your answers by editing the `2027-lab-8.qmd` file in your repository.
...
How do the estimates compare?: \_\_\_\_\_\_\_\_\_

Is the parallel trends assumption satisfied? \_\_\_\_\_\_\_\_\_
:::
...
::: {#Q2 .callout-note icon="false"}
## YOUR ANSWER Q2:

Show your work below

```{r}
#| label: copy data, mutate treatments, bind, predicts and average.
# (1) copy data


# (2, 3) mutate data to fix treatment (one of each) & bind data into single dataset


# (4) Model the outcome, and fit it using the original (unmutated) data. Show the coefficient estimates with broom::tidy() 


# (5) Use the model fit to predict the outcomes in the test data, Use broom::augment to predict the response for each row of the data


# (6) average the predicted outcomes and compute the contrast.
...
Knowing a lot about causal inference (and [Simpson's Paradox](https://en.wikipedia.org/wiki/Simpson's_paradox)), you decide to run a fixed effect model, adding the city’s indicator as a dummy variable to your model. The fixed effect model controls for city specific characteristics that are constant in time, so if a city is less open to your product, it will capture that.

::: {#Q3 .callout-note icon="false"}
## YOUR ANSWER Q3:
...
```{r}
#| label: build a double robust estimate using the best model
# show all your work
...
```{r}
#| label: bootstrap CI estimates for the model estimates
# show all your work
...
-   [**bullet #1**]{.underline}: (comment on the comparison of the robust causal estimate and the mean differences)

-   [**bullet #2**]{.underline}: (comment on omitted variable bias and small sample size bias in context)
:::
...
```{r}
#| label: create customer data with/without intervention
#
set.seed(8740)
n <- 2000; p <- 5;
...
```{r}
#| label: estimate the caual effect two ways
#| eval: false
...
```{r}
#| label: make two copies of new_dat and
#| eval: false

# (1) mutate one setting the intervention D=1
new_dat1 <-❓
...
```{r}
#| label: use the linear model (ols) to predict the responses under the two treatments
#| eval: false
# (2)
predicted1 <-❓
...
```{r}
#| label: combine the columns containing the predicted responses to create a two column dataframe, and
#| eval: false
# (3, 4)
...
```{r}
#| label: load weight-smoking data
nhefs <- 
...
```{r}
#| label: create mean difference in effects table
# (1)
...
```{r}
#| label: create normalization recipe
#| eval: false
# (2)
...
```{r}
#| label: calculate un-corrected matching estimate
# (3)
...
```{r}
#| label: calculate correction terms and revise estimate
#(4)
...
```{r}
#(5)
#| label: estimate the effect of smoking cessation on weight gain
...
::: {#Q7 .callout-note icon="false"}
## YOUR ANSWER Q7:

*You can write it here, or write it by hand on a separate piece of paper, scan the paper and add the image to your project.*
:::
...
```{r}
#| label: estimate the two stage DiD using the did2s package
...
I will pull (copy) everyone's repository submissions at 5:00pm on the Tuesday following class, and I will work only with these copies, so anything submitted after 5:00pm will not be graded. (**don't forget to commit and then push your work!**)
:::
