...
1.  build and compare ETS(A,A,A) and ETS(M,M,M) models. Use log(sales) as the outcome. Use `workflows` and `modeltime::exp_smoothing`, setting the error, trend and season argument explicitly for each model (see the help for `modeltime::exp_smoothing` ). For ETS(M,M,M), set damping = "damped".
