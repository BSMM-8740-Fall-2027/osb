...
::: {.callout-note appearance="simple" icon="false"}
\(2\) use ecom_data to model the "reduced form" stage by replacing log(total_price) with shipping_cost.
...
```{r}
reduced_form <- lm(log(quantity) ~ shipping_cost + factor(region) + factor(month) +
                               log(income) + age, data = ecom_data)
...
::: {.callout-note appearance="simple" icon="false"}
\(3\) use ecom_data to model the "first stage" by regressing log(total_price) on shipping_cost
...
```{r}
first_stage <- lm(log(total_price) ~ shipping_cost, data = ecom_data)
...
::: {.callout-note appearance="simple" icon="false"}
\(4\) estimate elasticity by dividing the "reduced form" by the "first stage" coefficients
...
NOTE: the naive model gives the wrong sign for elasticity, while the iV estimate is close to the actual value of -1.5. Note the IV gives the LATE not the ATE.
:::
...
-   Instruments weakly correlated with endogenous variable
