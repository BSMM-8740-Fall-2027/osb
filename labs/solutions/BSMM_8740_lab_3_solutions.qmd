...
Since no points lie outside the dashed lines of the *Influential Observations* chart, there are no overly influential observations.
:::
...
The variance inflation factor (VIF) is *moderate* (between 5-10) for `rad` and `tax`, suggesting these may be redundant for predicting the outcome (`medv`).
:::
...
-   now the coefficient of `price1` is correct (should be -0.5) while the coefficient of `price0` is not (it should be -1.0)
...
There are 11 predictors at this stage in cars_23. The 12th row in the summary tibble is the outcome.
:::
...
There are now **45 columns** in the data and 45 predictors in the recipe.

This is because the recipe converted the factors to dummy variables, increasing the number of predictors.
:::
