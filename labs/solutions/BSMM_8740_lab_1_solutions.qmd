...
Overall, the won-loss record can be computed as follows:
...
The overall won/loss record during 2004-2012 is: won 728, lost 730
:::
...
mets_ben %>% 
  ...
  dplyr::summarize('better than expected' = sum(W >= W_hat), 'worse than expected' = sum(W < W_hat))
```
...
The Mets were better than expected in 22 seasons.

The Mets were worse than expected in 40 seasons
:::
...
```{r}
# for this we just need to sort the number of wins in descending order using met_ben, 
# and filter for rows where W > W_hat
mets_ben |> dplyr::arrange(desc(W)) |> 
  dplyr::filter(W > W_hat)
```

The Mets seasons that were better than expected were: 2006, 2007, and 2012
:::
...
The share of customers receiving a discount is 74%
...
The share of customers that are 'loyal' is 75%
